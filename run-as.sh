...
if [ "$linux" = "true" ]; then
   JAVA_OPTS="$JAVA_OPTS -Djava.net.preferIPv4Stack=true -Xms256m -Xmx256m -server -XX:+UseNUMA"
fi
...
# Setup the classpath
GenericMBeanCLI="./GenericMBeanCLI.jar"
if [ ! -f "$GenericMBeanCLI" ]; then
    die "Missing required file: $GenericMBeanCLI"
fi
AS_BOOT_CLASSPATH="$GenericMBeanCLI"
...
while true; do
   ...
   if [ "$AS_STATUS" -eq 10 ]; then
      echo "Restarting Generic MBean CLI..."
   else
      exit $AS_STATUS
   fi
done
